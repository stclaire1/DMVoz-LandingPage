                    ...
                    <CarouselContent>
                        {(campaigns as Campaigns).map((campaign) => (
                            <CarouselItem key={campaign.videoId}>
                                <div className="flex flex-col items-center gap-3 md:flex-row md:gap-10 lg:px-3">
                                    <iframe
                                        className="rounded-lg aspect-square min-[450px]:aspect-video md:max-w-1/2 lg:max-w-3/5 2xl:max-w-1/2"
                                        src={`https://www.youtube.com/embed/${campaign.videoId}`}
                                        allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                                    ></iframe>
                                    <div className="flex flex-col gap-4 text-center md:text-start">
                                        <h3 className="mt-3 text-xl font-semibold lg:text-2xl 2xl:text-3xl">{campaign.title}</h3>
